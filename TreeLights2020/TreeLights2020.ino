...
const bool quarantine = true; // work from home conditions
...
void setup() {
  ...
  while ( WiFi.status() != WL_CONNECTED) {
    ...
    // fill with first color before WiFi is connected:
    if (tree.getPixelColor(0) != colors[1].green) {
      tree.fill(colors[1].green);
    } else {
      tree.clear();
    }
    tree.show();
